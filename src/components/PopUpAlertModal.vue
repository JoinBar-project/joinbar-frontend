...
<template>
  <transition name="modal-fade">
    <div v-if="visible" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50" @click.self="onCancel">
      <div class="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md text-center space-y-4">
        ...
        <div class="flex justify-center gap-4 pt-2">
          ...
          <button
            @click="onConfirm"
            class="px-4 py-2 rounded text-white font-semibold transition transform bg-[var(--color-primary-red)] hover:scale-105">
            {{ confirmText }}
          </button>
        </div>
      </div>
