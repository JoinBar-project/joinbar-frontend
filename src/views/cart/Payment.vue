...
const { createOrder, confirmPayment, simulatePayment, isLoading: orderLoading, error: orderError, clearError: clearOrderError } = useOrder();
...
const totalPrice = computed(() => cartItems.value.reduce((acc, item) => acc + item.price * item.quantity, 0).toLocaleString());

const canSubmit = computed(() => {
  return paymentMethod.value && !isSubmitting.value && !orderLoading.value && isCustomerInfoValid.value && cartItems.value.length > 0;
});
...
const isCustomerInfoValid = computed(() => {
  ...
  return name.trim() && phone.trim() && email.trim() && emailRegex.test(email);
});
