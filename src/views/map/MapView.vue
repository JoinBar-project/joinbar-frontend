<template>
  <div class="search-panel">
    <div ref="inputArea" class="input-group">
      ...
      <button
        @click="handleSearch"
        class="btn bg-[#decdd5] hover:bg-[#860914] text-white rounded-r-lg font-normal search-bt"
      >
        🔍 搜尋
      </button>
    ...
    <button @click="getCurrentLocation" class="font-normal btn place-now">
      📍 顯示我目前位置
    </button>
  </div>
...
function loadGoogleMapsScript() {
  return new Promise((resolve, reject) => {
    ...
    script.src = `https://maps.googleapis.com/maps/api/js?key=${import.meta.env.VITE_Maps_API_KEY}&libraries=places,marker&v=beta&solution_channel=GMP_CCS_complexmarkers_v3`;
    script.async = true;
    script.defer = true;
    script.onload = () => {
...
function searchNearbyBars(query, location, radius = 2000) {
  ...
  if (
    currentMarker &&
    (location.lat !== currentMarker.getPosition().lat ||
      location.lng !== currentMarker.getPosition().lng)
  ) {
    clearAllMarkers();
  ...
  const request = {
    ...
    radius: 1500,
    type:
      query.includes("酒吧") || query.includes("bar")
        ? ["bar", "liquor_store"]
        : undefined,
    keyword: query,
  };
    ...
    results.forEach((place) => {
      ...
      placesService.getDetails(
        ...
        (details, status) => {
          ...
          const isBarType =
            details.types &&
            (details.types.includes("bar") ||
              details.types.includes("liquor_store"));
          const nameMatches = /酒|bar|pub|雞尾酒|lounge/i.test(details.name);
          const reviewMatches = Array.isArray(details.reviews)
            ? details.reviews.some((r) =>
                /酒|bar|pub|雞尾酒|lounge/i.test(r.text)
              )
            : false;
        ...
        geocoder.geocode({ location }, (results, status) => {
          if (status === "OK" && results[0]) {
            ...
            infoWindow.setContent(
              `<strong>你現在的位置</strong><br/>${address}`
            );
          } else {
            infoWindow.setContent(
